...
LOG_ENCODING = "utf-8"
BYTES_PRE_THREAD = 256 << 20
MAX_PROCESSES = 4
MAX_THREADS_PER_PROCESSOR = 8
...
def convert_log(log_dir: str = "", *, log_path: str = "") -> None:
    ...
    if not log_path:
        ...
        if len(log_files) <= 0:
            ...
            print(
                "[ERROR] Multiple tracking control log files" f'found in "{LOG_DIR}"!'
            )
            return
    ...
    print(f'[INFO] Converting "{os.path.relpath(log_dir, SCRIPT_DIR)}"...')
    ...
    n_slices = ceil(total_bytes / BYTES_PRE_THREAD)
    ...
    if n_slices == 1:
        df_output = convert_slice(pos_slices[0])
    else:
        n_threads = min(n_slices, MAX_THREADS_PER_PROCESSOR)
        with ThreadPoolExecutor(n_threads) as executor:
            df_output = pd.concat(
                executor.map(convert_slice, pos_slices), ignore_index=True, copy=False
            )
    ...
    print(
        f"[INFO] Generated {len(df_output):,d} rows of data from "
        f'"{os.path.relpath(log_path, SCRIPT_DIR)}".'
    )
...
if __name__ == "__main__":
    ...
    if "DATA_PATH" in locals() and DATA_PATH:
        convert_log(log_path=DATA_PATH)
    else:
        ...
        n_processes = min(n_log_dirs, MAX_PROCESSES)
        with ProcessPoolExecutor(n_processes) as executor:
            executor.map(convert_log, LOG_DIRS)
