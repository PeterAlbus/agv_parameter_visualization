...
if "DATA_PATH" not in locals():
    ...
    if len(log_files) <= 0:
        raise RuntimeError(f"No log files found in {LOG_DIR!r}!")
    elif len(log_files) > 1:
        raise RuntimeError(f"Multiple log files found in {LOG_DIR!r}!")
    DATA_PATH = os.path.join(LOG_DIR, log_files[0])
elif TYPE_CHECKING:
    DATA_PATH = ""  # for type checking

print(f"Converting {DATA_PATH!r}...")
...
print(f"Generated {min_length:,} lines of data.")
